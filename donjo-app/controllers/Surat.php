...
class Surat extends Admin_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            return datatables()->of(FormatSurat::kunci(FormatSurat::KUNCI_DISABLE)->orderBy('favorit', 'desc')->latest('updated_at'))
                ...
                ->addColumn('jenis_label', static function ($row) {
            ...
            return json([
                'results' => collect($surat->items())
                    ->map(static function ($item) {
                        return [
                            'id'   => $item->url_surat,
                            'text' => (in_array($item->jenis, FormatSurat::RTF) ? '[RTF]' : '[TinyMCE]') . " - {$item->nama}",
                        ];
                    }),
                'pagination' => [
    ...
    public function pdf($preview = false)
    {
        ...
        if ($cetak) {
            $url_surat = $cetak['surat']['url_surat'] ?? null;
            if ($this->penomoran_surat_model->nomor_surat_duplikat('log_surat', $cetak['input']['nomor'], $url_surat)) {
                $nomor_baru              = $this->penomoran_surat_model->dapatkan_nomor_bebas('log_surat', $cetak['input']['nomor'], $url_surat);
                $cetak['input']['nomor'] = $nomor_baru;
                $cetak['no_surat']       = $nomor_baru;
                $this->session->log_surat = $cetak;
            }
            $id_pamong = $this->ttd($this->request['pilih_atas_nama'], $this->request['pamong_id']);
            $pamong    = Pamong::find($id_pamong);
            $log_surat = [
    ...
    private function cetak_doc($url)
    {
        ...
        if ($this->penomoran_surat_model->nomor_surat_duplikat('log_surat', $log_surat['no_surat'], $url)) {
            $nomor_baru            = $this->penomoran_surat_model->dapatkan_nomor_bebas('log_surat', $log_surat['no_surat'], $url);
            $log_surat['no_surat'] = $nomor_baru;
            $_POST['nomor']        = $nomor_baru;
        }
        $id                        = $_POST['nik'];
        ...
        switch ($url) {
            ...
            case 'surat_kuasa':
            case 'surat_jual_beli':
            case 'surat_hibah':
            case 'surat_pernyataan_jual_beli':
            ...
            default:
                if (! $id) {
                    $id = $_POST['id_pemberi_kuasa'] ?? $_POST['id_penerima_kuasa'] ?? $_POST['id_penjual'] ?? $_POST['id_pemohon'] ?? null;
                }
                break;
        }
