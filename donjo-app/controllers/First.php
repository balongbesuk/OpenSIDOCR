...
class First extends Web_Controller
{
    ...
    */
        ...
        public function statistik($stat = 0, $tipe = 0)
    {
        ...
        // Blok kode kustom untuk perkembangan-penduduk
        if ($stat === 'perkembangan-penduduk') {
            $data['heading'] = 'Perkembangan Penduduk';
    ...
    // Tambahkan method baru ini di bawah fungsi statistik() di atas
    public function perkembangan_penduduk()
    {
        $this->statistik('perkembangan-penduduk');
    }
    ...
    {
        $tahun_param = $this->input->get('tahun') ?: '5_tahun';
        
        $this->load->model('penduduk_model');
        $this->load->model('keluarga_model');
        
        $config_id = identitas('id');
        $total_penduduk_sekarang = $this->db->where('status_dasar', 1)->where('config_id', $config_id)->count_all_results('tweb_penduduk');
        $total_keluarga_sekarang = $this->db->where('config_id', $config_id)->count_all_results('tweb_keluarga');

        if (preg_match('/(\d+)_tahun/', $tahun_param, $matches)) {
            $rentang_tahun = (int) $matches[1];
            $tahun_mulai = date('Y') - ($rentang_tahun - 1);
            $tahun_akhir = date('Y');
        ...
        return $this->output->set_content_type('application/json')->set_output(json_encode(['error' => 'Invalid filter']));
    }
