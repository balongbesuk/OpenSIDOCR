...
class Surat_model extends MY_Model
{
    ...
    private function list_penduduk_ajax_sql($cari = '', $filter = [])
    {
        ...
        // Default: hanya penduduk hidup (status_dasar = 1), kecuali ditentukan lain
        // (misal surat kematian mengirim status_dasar = 2 melalui form_isian)
        $status_dasar = !empty($filter['status_dasar']) ? $filter['status_dasar'] : 1;
        $this->db->where('u.status_dasar', $status_dasar);

        if (!empty($filter['sex'])) {
            $this->db->where('sex', $filter['sex']);
        }

        if (!empty($filter['kk_level'])) {
            $this->db->where('kk_level', $filter['kk_level']);
        }

        if (!empty($filter['bersurat'])) {
    ...
    public function format_data_surat(&$data)
    {
        ...
        $this->get_pamong_wilayah($data);
    }

    public function get_pamong_wilayah(&$data)
    {
        $dusun = $data['dusun'] ?? '';
        $rw    = $data['rw'] ?? '';
        $rt    = $data['rt'] ?? '';
        ...
        $data['jabatan_ketua_rt']     = (! empty($rt) && $rt != '-' && ! empty($rw) && $rw != '-') ? "Ketua RT {$rt} / RW {$rw}" : '-';
        $data['jabatan_ketua_rw']     = (! empty($rw) && $rw != '-') ? "Ketua RW {$rw}" : '-';
        $sebutan_dusun                = set_ucwords(setting('sebutan_dusun') ?: 'Dusun');
        $data['jabatan_kepala_dusun'] = (! empty($dusun)) ? "Kepala {$sebutan_dusun} " . set_ucwords($dusun) : '-';

        $data['jabatan_rt']    = $data['jabatan_ketua_rt'];
        $data['jabatan_rw']    = $data['jabatan_ketua_rw'];
        $data['jabatan_dusun'] = $data['jabatan_kepala_dusun'];

        if (! empty($dusun)) {
            // Kepala Dusun
            $kadus = $this->config_id('c')
                ->select('p.nama')
                ->from('tweb_wil_clusterdesa c')
                ->join('tweb_penduduk p', 'c.id_kepala = p.id')
                ->where('c.dusun', $dusun)
                ->where_in('c.rw', ['0', '-'])
                ->where_in('c.rt', ['0', '-'])
                ->get()->row();
            if ($kadus) {
                $data['nama_kepala_dusun'] = set_ucwords($kadus->nama);
            }
        }

        if (! empty($dusun) && ! empty($rw) && $rw != '-') {
            // Ketua RW
            $ketua_rw = $this->config_id('c')
                ->select('p.nama')
                ->from('tweb_wil_clusterdesa c')
                ->join('tweb_penduduk p', 'c.id_kepala = p.id')
                ->where('c.dusun', $dusun)
                ...
                ->get()->row();
            if ($ketua_rw) {
                $data['nama_ketua_rw'] = set_ucwords($ketua_rw->nama);
            }
        }

        if (! empty($dusun) && ! empty($rw) && ! empty($rt) && $rt != '-') {
            // Ketua RT
            $ketua_rt = $this->config_id('c')
                ->select('p.nama')
                ->from('tweb_wil_clusterdesa c')
                ->join('tweb_penduduk p', 'c.id_kepala = p.id')
                ->where('c.dusun', $dusun)
                ...
                ->get()->row();
            if ($ketua_rt) {
                $data['nama_ketua_rt'] = set_ucwords($ketua_rt->nama);
            }
        }
    }
    ...
    public function surat_rtf($data)
    {
        ...
        if (is_file($file)) {
            ...
            $array_replace = [
                ...
                // Data Pengurus Wilayah (RT, RW, Dusun)
                '[nama_ketua_rt]'        => $individu['nama_ketua_rt'] ?? '-',
                '[nama_ketua_rw]'        => $individu['nama_ketua_rw'] ?? '-',
                '[nama_kepala_dusun]'    => $individu['nama_kepala_dusun'] ?? '-',
                '[jabatan_ketua_rt]'     => $individu['jabatan_ketua_rt'] ?? '-',
                '[jabatan_ketua_rw]'     => $individu['jabatan_ketua_rw'] ?? '-',
                '[jabatan_kepala_dusun]' => $individu['jabatan_kepala_dusun'] ?? '-',
                '[jabatan_rt]'           => $individu['jabatan_rt'] ?? '-',
                '[jabatan_rw]'           => $individu['jabatan_rw'] ?? '-',
                '[jabatan_dusun]'        => $individu['jabatan_dusun'] ?? '-',

                // Data RTM
                '[bdt]' => $individu['bdt'] ?? '-',
            ];
            $buffer = str_replace(array_keys($array_replace), array_values($array_replace), $buffer);
            $buffer = case_replace('[nama_ketua_rt]', $individu['nama_ketua_rt'] ?? '-', $buffer);
            $buffer = case_replace('[nama_ketua_rw]', $individu['nama_ketua_rw'] ?? '-', $buffer);
            $buffer = case_replace('[nama_kepala_dusun]', $individu['nama_kepala_dusun'] ?? '-', $buffer);
            $buffer = str_ireplace('[jabatan_ketua_rt]', $individu['jabatan_ketua_rt'] ?? '-', $buffer);
            $buffer = str_ireplace('[jabatan_ketua_rw]', $individu['jabatan_ketua_rw'] ?? '-', $buffer);
            $buffer = str_ireplace('[jabatan_kepala_dusun]', $individu['jabatan_kepala_dusun'] ?? '-', $buffer);
            $buffer = str_ireplace('[jabatan_rt]', $individu['jabatan_rt'] ?? '-', $buffer);
            $buffer = str_ireplace('[jabatan_rw]', $individu['jabatan_rw'] ?? '-', $buffer);
            $buffer = str_ireplace('[jabatan_dusun]', $individu['jabatan_dusun'] ?? '-', $buffer);
    ...
    public function get_last_nosurat_log($url)
    {
        ...
        if ($this->setting->penomoran_surat == 2 && empty($data['kode_surat'])) {
            $surat              = $this->get_surat($url);
            $data['kode_surat'] = $surat['kode_surat'];
        }
        $ket = [
            1 => 'Terakhir untuk semua surat layanan: ',
            2 => "Terakhir untuk kode surat {$data['kode_surat']}: ",
