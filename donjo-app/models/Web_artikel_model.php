...
class Web_artikel_model extends MY_Model
{
    ...
    public function insert($cat = 1)
    {
        ...
        // Batasi judul menggunakan teks polos dan filter judi
        $data['judul'] = filter_konten_judi(judul($data['judul']));
        ...
        if ($outp) {
            ...
            if ($cat != AGENDA && (!isset($data['enabled']) || $data['enabled'] == 1)) {
                $this->kirim_onesignal($id_artikel);
            }
        }
    ...
    public function update($cat, $id = 0)
    {
        ...
        if ($cat == AGENDA) {
            ...
            $old_artikel = $this->db->select('enabled')->where('id', $id)->get('artikel')->row_array();

            $outp = $this->config_id()->where('a.id', $id)->update('artikel a', $data);

            if ($outp && $old_artikel && $old_artikel['enabled'] != 1 && isset($data['enabled']) && $data['enabled'] == 1) {
                $this->kirim_onesignal($id);
            }
        }
    ...
    public function artikel_lock($id = 0, $val = 1)
    {
        ...
        if ($outp && $old_artikel && $old_artikel['enabled'] != 1 && $val == 1) {
            $this->kirim_onesignal($id);
        }
    ...
    private function kirim_onesignal($id_artikel)
    {
        $artikel = $this->db->select('a.judul, a.slug, a.tgl_upload, a.id_kategori, k.slug as kat_slug, YEAR(a.tgl_upload) as thn, MONTH(a.tgl_upload) as bln, DAY(a.tgl_upload) as hri')
            ->from('artikel a')
            ->join('kategori k', 'a.id_kategori = k.id', 'left')
        ...
        if (in_array((int)$artikel['id_kategori'], [999, 1000])) {
            return;
        }

        $slug_url = buat_slug($artikel);
        $url = site_url('artikel/' . $slug_url);

        $fields = [
            'app_id' => '56a523c8-0ab9-4702-9f18-2ad17efa97e3',
            ...
            'contents' => [
                'en' => 'Klik untuk membaca artikel selengkapnya di website resmi desa.',
                'id' => 'Klik untuk membaca artikel selengkapnya di website resmi desa.'
            ],
            'url' => $url,
        ...
        $headers = [
            'Authorization: Key os_v2_app_k2sshsakxfdqfhyyflix56ux4pa5zurjwdwetcm6736bxgi7entl7snztbye7dppswucvebmui6nttviie5lxxoplyaupokehlvkx4y',
