...
class Penomoran_surat_model extends MY_Model
{
        ...
        switch ($type) {
            ...
            case 'log_surat':
                if ($setting == 1) {
                    ...
                    $surat_format = $this->db->select('kode_surat')->get_where('tweb_surat_format', ['url_surat' => $url])->row_array();
                    $kode_surat   = $surat_format['kode_surat'] ?? '';

                    $this->config_id('l')
                        ->select('*, f.nama, f.kode_surat, l.id id_surat')
                        ->from("{$type} l")
                        ->join('tweb_surat_format f', 'f.id=l.id_format_surat', 'RIGHT')
                        ->where('f.kode_surat', $kode_surat)
    ...
    /**
     * Dapatkan nomor urut bebas yang belum digunakan (jika nomor_surat bentrok, otomatis +1)
     *
     * @param string      $type
     * @param string|int  $nomor_surat
     * @param string|null $url
     *
     * @return string nomor surat bebas
     */
    public function dapatkan_nomor_bebas($type, $nomor_surat, $url = null)
    {
        $nomor   = (int) $nomor_surat;
        $panjang = (int) $this->setting->panjang_nomor_surat;
        if ($panjang <= 0) {
            $panjang = strlen((string) $nomor_surat);
        }

        while ($this->nomor_surat_duplikat($type, str_pad((string) $nomor, $panjang, '0', STR_PAD_LEFT), $url)) {
            $nomor++;
        }

        return str_pad((string) $nomor, $panjang, '0', STR_PAD_LEFT);
    }
}
