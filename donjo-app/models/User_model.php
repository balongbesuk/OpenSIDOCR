...
class User_model extends MY_Model
{
    ...
    public function siteman()
    {
        ...
        if (($user->id_grup == $this->user_model->id_grup(UserGrup::REDAKSI)) && ($this->setting->offline_mode >= 2)) {
            ...
            $idleTimeout  = $timeoutMinutes * 60;
            $lastActivity = $user->last_login ? strtotime($user->last_login) : 0;
            $isActive     = ! empty($user->session) && ((time() - $lastActivity) < $idleTimeout);

            if ($isActive) {
                $durasiTeks = $timeoutMinutes >= 60 ? ($timeoutMinutes / 60) . ' jam' : $timeoutMinutes . ' menit';
                $this->session->siteman = -3;
                $this->session->set_flashdata('session_active_error', "Akun sedang aktif di perangkat lain. Silakan tunggu {$durasiTeks} tanpa aktivitas atau gunakan tombol Paksa Logout.");
