...
class Keluar_model extends MY_Model
{
    ...
    public function log_surat($data_log_surat)
    {
        ...
        */
            $this->config_id()->select('id')->from('log_surat')->where('nama_surat', $nama_surat);
            if ($this->db->field_exists('deleted_at', 'log_surat')) {
                $this->db->where('deleted_at is null');
            }
            $log_id = $this->db->limit(1)->get()->row()->id;
        } else { // Cetak:
    ...
    public function delete($id = '')
    {
        ...
        if ($arsip['status'] == 0 || ! $this->db->field_exists('deleted_at', 'log_surat')) { // jika masih draft/tidak ada deleted_at maka hapus data
