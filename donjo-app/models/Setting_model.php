...
define('maxPhpVersion', '8.5.0');
define('minMySqlVersion', '5.6.0');
define('maxMySqlVersion', '9.0.0');
    ...
    */
    ...
    public function cekKebutuhanSistem()
    {
        ...
        foreach ($sistem as $value) {
            ...
            $ini_val      = ini_get($key);
            $ini_bytes    = $this->parseSize($ini_val);
            $target_bytes = $this->parseSize($val);

            $data[$key] = [
                'v'      => $val,
                $key     => $ini_val,
                'result' => ($kondisi === '>=') ? ($ini_bytes >= $target_bytes) : ($ini_bytes == $target_bytes),
            ];
        }
