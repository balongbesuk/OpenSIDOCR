...
class Laporan_bulanan_model extends MY_Model
{
    ...
    public function perbaikiLogKeluarga()
    {
        ...
        $has_pk_kel = $this->db->query("SHOW KEYS FROM log_keluarga WHERE Key_name = 'PRIMARY'")->num_rows();
        if (!$has_pk_kel) {
            $this->db->query("ALTER TABLE log_keluarga MODIFY id INT AUTO_INCREMENT, ADD PRIMARY KEY (id)");
            $this->db->query("ALTER TABLE log_keluarga ADD INDEX idx_id_kk (id_kk)");
        ...
        $this->db->query("
            DELETE FROM log_penduduk 
            WHERE kode_peristiwa IN (1, 5) 
              AND EXISTS (
                  SELECT 1 FROM (SELECT * FROM log_penduduk) as l2 
                  WHERE l2.id_pend = log_penduduk.id_pend 
                    AND l2.kode_peristiwa IN (2, 3, 4) 
                    AND l2.id < log_penduduk.id
              ) 
              AND EXISTS (
                  SELECT 1 FROM tweb_penduduk p 
                  WHERE p.id = log_penduduk.id_pend 
                    AND p.status_dasar IN (2, 3, 4)
              )
        ");

        // 3. Perbaiki kode_peristiwa yang NULL untuk penduduk aktif agar terhitung kembali di laporan bulanan
        $this->db->query("
            UPDATE log_penduduk 
        ...
        // 4. Bersihkan log pergi (event 6) untuk penduduk yang berstatus dasar aktif (Hidup)
        $this->db->query("
            DELETE l FROM log_penduduk l
            JOIN tweb_penduduk p ON l.id_pend = p.id
            WHERE l.kode_peristiwa = 6 AND p.status_dasar = 1
        ");

        // 5. Bersihkan duplikat log peristiwa keluarga (id_kk, id_peristiwa yang sama)
        $this->db->query("
            DELETE l FROM log_keluarga l
            LEFT JOIN (
                SELECT MIN(id) as min_id 
                FROM log_keluarga 
                GROUP BY id_kk, id_peristiwa
            ) as tmp ON l.id = tmp.min_id
