...
class Keluarga_model extends MY_Model
{
    ...
    */
    ...
    public function list_data($o = 0, $page = 1)
    {
        ...
        if (! $this->input->get('id_cb') && $page > 0) {
        ...
        //Formating Output
        $j = isset($paging) ? ($paging->offset ?: 0) : 0;
    ...
    */
    ...
    public function rem_anggota($kk = 0, $id = 0)
    {
        ...
        $temp['no_kk_sebelumnya'] = $kk ? ($kel->no_kk ?? null) : null; // Tidak simpan no kk kalau keluar dari keluarga
        $temp['id_kk']            = 0;
        $temp['kk_level']         = 0;
        $temp['updated_at']       = date('Y-m-d H:i:s');
        $temp['updated_by']       = $this->session->user;
        ...
        if ($kel && $kel->id) { $this->log_keluarga($kel->id, LogKeluarga::ANGGOTA_KELUARGA_PECAH, $id); }
    }
    ...
    {
        return $this->config_id()
            ->select('id, nik, nama, id_kk, kk_level, status_dasar')
            ->where('id', $id)
            ->get('tweb_penduduk')
            ->row_array();
    }
