...
class Penduduk_log_model extends MY_Model
{
    ...
    public function kembalikan_status_pergi($id_log)
    {
        ...
        // Cek tgl lapor
        // tampilkan hanya jika beda bulan/tahun lapor
        $tgl_lapor    = Carbon::parse($log['tgl_lapor'])->format('Y-m');
        $tgl_sekarang = Carbon::now()->format('Y-m');
        if ($tgl_lapor > $tgl_sekarang) {
            session_error('Tidak dapat mengubah status dasar penduduk, karena tanggal lapor peristiwa lebih besar dari bulan sekarang.');
        ...
        if (in_array($log->kode_peristiwa, [LogPenduduk::PINDAH_KELUAR, LogPenduduk::TIDAK_TETAP_PERGI])) {
            ...
            $pend = $this->penduduk_model->get_penduduk($log->id_pend);
            ...
            $logPenduduk = [
                ...
                'id_pend'                  => $log->id_pend,
                'no_kk'                    => $pend['no_kk'] ?? null,
                'nama_kk'                  => $pend['kepala_kk'] ?? null,
                'ref_pindah'               => $log->ref_pindah ?? null,
                'created_by'               => auth()->id,
